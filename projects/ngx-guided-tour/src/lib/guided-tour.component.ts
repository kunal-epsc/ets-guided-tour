...
import {
    GuidedTourService
} from './guided-tour.service';
import {
    WindowRefService
} from "./windowref.service";

@Component({
    selector: 'ngx-guided-tour',
    template: `
    <div [ngClass]="tourTheme === 'DARK' ? 'dark-mode' : 'light-mode'">
    ...
    <div *ngIf="currentTourStep && !isOrbShowing">
        <div #tourStep *ngIf="currentTourStep" class="tour-step tour-{{ currentTourStep.orientation }}" [ngClass]="{
            ...
            && !guidedTourService.onResizeMessage" class="tour-progress-indicator">
                ...
                <button *ngIf="!guidedTourService.onResizeMessage && !currentTourStep.showRestart"
                ...
                <div class="tour-buttons">
                    <button *ngIf="!guidedTourService.onResizeMessage && currentTourStep.showBack"
                        (click)="guidedTourService.backAction()" class="skip-button link-button">
                        Back
                    </button>
                    <button *ngIf="!guidedTourService.onResizeMessage && currentTourStep.showRestart"
                        (click)="guidedTourService.restartAction()" class="skip-button link-button">
                        Restart Tour
                    </button>
                    <button *ngIf="!guidedTourService.onLastStep 
                    && !guidedTourService.onResizeMessage" class="next-button" (click)="guidedTourService.nextStep()">
                        {{ nextText }}
                    ...
                    <button *ngIf="guidedTourService.onLastStep  
                    && currentTourStep.showDone !== false" class="next-button" (click)="guidedTourService.nextStep()">
                        {{ doneText }}
                    </button>
                    ...
                    <button *ngIf="currentTourStep.showEdit !== false && !guidedTourService.onResizeMessage"
                        class="back-button link-button" (click)="guidedTourService.editAction()">
                        {{!!currentTourStep.editText ? currentTourStep.editText: 'Edit'}}
                        <ng-container *ngIf="progressIndicatorLocation === progressIndicatorLocations.InsideNextButton">
                            <ng-container *ngTemplateOutlet="progress"></ng-container>
                        </ng-container>
                    </button>
    ...
    <!-- <ng-template #progress>
        ...
        context: { currentStepNumber: progressIndicator.split('/')[0], totalSteps: progressIndicator.split('/')[1] }
        "></ng-container>
    </ng-template>
    <ng-template #defaultProgressIndicator let-currentStepNumber="currentStepNumber" let-totalSteps="totalSteps">
        <ng-container *ngIf="progressIndicatorLocation === progressIndicatorLocations.InsideNextButton">&nbsp;
        </ng-container>{{ currentStepNumber }}/{{ totalSteps }}
    </ng-template> -->
    </div>
`,
...
export class GuidedTourComponent implements AfterViewInit, OnDestroy {
    ...
    @Input() public tourProgress ? : string = '';
    ...
    @Input() public tourTheme: TourTheme;

    @ViewChild('tourStep', {
        static: false
    }) public tourStep: ElementRef;
    ...
    // called when we hover over an orb
